...
class SdrPiDisplay:
  def __init__(self, display_width = 128, display_height = 64, addr = 0x3c):
    ...
    self.dipslay_height = display_height
    self.addr = addr
    ...
    # Set up oled screen
    self.oled = adafruit_ssd1306.SSD1306_I2C(self.display_width, self.dipslay_height, self.i2c, addr=self.addr)
  ...
  def update_display(self, config):
    ...
    self.draw.rectangle((0, 0, self.display_width//2, self.dipslay_height), fill=0) # Erase area
    self.draw.text((6, self.dipslay_height//2-font_height//2), mode, font=self.font, fill=255)

    # Update channel/frequency
    channel = config.get_mode().get_channel_display()
    (font_width, font_height) = self.font.getsize(channel)
    self.draw.rectangle((self.display_width//2, 0, self.display_width, self.dipslay_height), fill=0)
    ...
    if config.editing == "freq":
      ...
      self.draw.rectangle((self.display_width//2+4, 4, self.display_width-4, self.dipslay_height-4), fill=None, outline=0)
      self.draw.rectangle((4, 4, self.display_width//2-4, self.dipslay_height-4), fill=None, outline=255)
