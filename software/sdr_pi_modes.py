...
class ModeFM(SdrPiMode):
  ...
  def activate(self):
    self.ps_rtl_fm = subprocess.Popen(('rtl_fm', '-M', 'wbfm', '-f', '%.1fM' % (self.freq)), stdout=subprocess.PIPE)
...
class ModeCBFM(SdrPiMode):
  ...
  def activate(self):
    self.ps_rtl_fm = subprocess.Popen(('rtl_fm', '-M', 'fm', '-r', '8k', '-f', '%.3fM' % (self.channels[self.channel+1])), stdout=subprocess.PIPE)
...
class ModeAirbandPresets(SdrPiMode):
  ...
  def activate(self):
    self.ps_rtl_fm = subprocess.Popen(('rtl_fm', '-M', 'am', '-r', '8k', '-f', '%.3fM' % (self.channels[self.channel][0])), stdout=subprocess.PIPE)
    self.ps_sox = subprocess.Popen(('play', '-r', '8k', '-t', 'raw', '-e', 's', '-b', '16', '-c', '1', '-V1', '-'), stdin=self.ps_rtl_fm.stdout)
